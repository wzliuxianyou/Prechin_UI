...
static rt_err_t rt_pulse_encoder_control(struct rt_device *dev, int cmd, void *args)
{
    ...
    switch (cmd)
    {
		...
		*add by lxy
		*************************************/
		case PULSE_ENCODER_CMD_GET_DIRECTTION:
			result = pulse_encoder->ops->get_direction(pulse_encoder, args);
			break;
	    default:
	        result = -RT_ENOSYS;
	        break;
...
void rt_hw_encoder_isr(struct rt_pulse_encoder_device *pulse_encoder)
{
	/* invoke callback */
	if (pulse_encoder->parent.rx_indicate != RT_NULL)
	{
		pulse_encoder->parent.rx_indicate(&pulse_encoder->parent, 0);		
	}
}
