...
rt_err_t pulse_encoder_init(struct rt_pulse_encoder_device *pulse_encoder)
{
    ...
    stm32_device->tim_handler.Init.Prescaler = 1;
...
rt_err_t pulse_encoder_control(struct rt_pulse_encoder_device *pulse_encoder, rt_uint32_t cmd, void *args)
{
    ...
    switch (cmd)
    {
	    case PULSE_ENCODER_CMD_ENABLE:
	        ...
	        HAL_TIM_Encoder_Start(&stm32_device->tim_handler, TIM_CHANNEL_1);
	        HAL_TIM_Encoder_Start_IT(&stm32_device->tim_handler, TIM_CHANNEL_1);
	        break;
	    case PULSE_ENCODER_CMD_DISABLE:
	        ...
	        HAL_TIM_Encoder_Stop(&stm32_device->tim_handler, TIM_CHANNEL_1);
	        HAL_TIM_Encoder_Stop_IT(&stm32_device->tim_handler, TIM_CHANNEL_1);		
	        break;
	    default:
	        result = -RT_ENOSYS;
	        break;
...
/**** add by lxy ******/
rt_err_t pulse_encoder_get_direction(struct rt_pulse_encoder_device *pulse_encoder, void *args)
{
    struct stm32_pulse_encoder_device *stm32_device;
    stm32_device = (struct stm32_pulse_encoder_device*)pulse_encoder;
	*(rt_uint8_t *)args = (rt_uint8_t)(__HAL_TIM_IS_TIM_COUNTING_DOWN(&stm32_device->tim_handler));
	return RT_EOK;
}
...
void pulse_encoder_update_isr(struct stm32_pulse_encoder_device *device)
{
    ...
    if (__HAL_TIM_GET_FLAG(&device->tim_handler, TIM_FLAG_CC1) != RESET)
    {
		...
		/*************add by lxy*************************/	
		rt_hw_encoder_isr(&device->pulse_encoder);		
    }
...
static const struct rt_pulse_encoder_ops _ops =
{
    ...
    /*************add by lxy*************************/
    .get_direction = pulse_encoder_get_direction,
};
