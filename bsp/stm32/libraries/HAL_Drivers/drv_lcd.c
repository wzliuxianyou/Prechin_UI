...
void drv_lcd_fill(uint32_t color, void *dst, int16_t xSize, int16_t ySize, int16_t OffLine) 
{
    ...
    DMA2D->OPFCCR = DMA2D_OUTPUT_RGB565;          /* Defines the number of pixels to be transfered */
    DMA2D->NLR = (uint32_t)(xSize << 16) | (uint16_t)ySize; /* Size configuration of area to be transfered */

	DMA2D->CR = DMA2D_R2M;						//拷贝固定颜色至显存
	DMA2D->CR |= DMA2D_CR_START;                /* Start the transmission */\
}

static void drv_lcd_DrawHLine(uint32_t color, int x_start, int x_end, int y_pos) 
{
 	uint32_t addr = (uint32_t)(LtdcHandle.LayerCfg[0].FBStartAdress) + 2 *(_lcd.lcd_info.width * y_pos + x_start);   
	int xSize = x_end - x_start;
    drv_lcd_fill(color, (void *)addr, xSize, 1, _lcd.lcd_info.width - xSize);
}

static void drv_lcd_DrawVLine(uint32_t color, int x_pos, int y_start, int y_end) 
{
 	uint32_t addr = (uint32_t)(LtdcHandle.LayerCfg[0].FBStartAdress) + 2 *(_lcd.lcd_info.width * y_start + x_pos); 
	int ySize = y_end - y_start;
    drv_lcd_fill(color, (void *)addr, 1, ySize, _lcd.lcd_info.width - 1);
}


static void drv_lcd_SetPixel(uint32_t color, int x_pos, int y_pos)
{
 	uint32_t addr = (uint32_t)(LtdcHandle.LayerCfg[0].FBStartAdress) + 2 *(_lcd.lcd_info.width * y_pos + x_pos); 
    drv_lcd_fill(color, (void *)addr, 1, 1, _lcd.lcd_info.width - 1);
}


const static struct rt_device_graphic_ops lcd_draw_ops =
{
    drv_lcd_SetPixel,
    RT_NULL,
    drv_lcd_DrawHLine,
...
#ifdef RT_USING_LTDC_DOUBLE_BUF
    	/**********************************************
    	* 配置行中断的行数为最后一行, 并使能行中断
    	**********************************************/
		HAL_LTDC_ProgramLineEvent(&LtdcHandle, LtdcHandle.Init.AccumulatedActiveH);
#endif
    	/***************************************************
    	* 配置优先级并开启总中断，中断中进行framebuffer切换
    	****************************************************/
...
static void stm32_DMA2D_Init(struct drv_lcd_device *lcd)
{
	...
	{
		LOG_E("HAL_DMA2D_ConfigLayer failed!\n");
	}
}
...
#ifdef RT_USING_LTDC_DOUBLE_BUF
static void LineIrq_entry(void *parameter)
{
    while (1)
    {
			...
			rt_sem_release(&_lcd.lcd_dmaSet);
    }
}
#endif
...
int drv_lcd_hw_init(void)
{
    ...
    /* malloc sdram memory for Triple Buffering */
    _lcd.lcd_info.framebuffer = rt_memheap_alloc(&sdram_heap, LCD_BUF_SIZE);
#ifdef RT_USING_LTDC_DOUBLE_BUF
		rt_thread_t  LineIrqThread = RT_NULL;
    _lcd.back_buf = rt_memheap_alloc(&sdram_heap, LCD_BUF_SIZE);
    _lcd.front_buf = rt_memheap_alloc(&sdram_heap, LCD_BUF_SIZE);
	...
	* 创建信号量通知dma2d
	*******************************************/
    result = rt_sem_init(&_lcd.lcd_dmaSet, "lcd_dmaSet", 0, RT_IPC_FLAG_FIFO);
    if (result != RT_EOK)
    {
        LOG_E("init semaphore failed!\n");
        result = -RT_ENOMEM;
	...
	* 创建线程监听行中断，进行双缓冲切换
	*******************************************/
    LineIrqThread = rt_thread_create("LineIrq", LineIrq_entry, RT_NULL, 1024, 20, 5);
    if (LineIrqThread != RT_NULL)
        rt_thread_startup(LineIrqThread);
	//添加私有数据
	device->user_data = (struct drv_lcd_device *)&_lcd;
#else
	...
	memset(_lcd.lcd_info.framebuffer, 0xFF, LCD_BUF_SIZE);
	//添加私有数据
	device->user_data = (struct rt_device_graphic_ops *)&lcd_draw_ops;
#endif
    ...
    /* register lcd device */
    rt_device_register(device, "lcd0", RT_DEVICE_FLAG_RDWR);
...
int lcd_test()
{
    ...
    lcd = (struct drv_lcd_device *)rt_device_find("lcd0");
